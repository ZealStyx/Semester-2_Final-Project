 ...
 * item systems.
 */
public final class GltfMapTestScene extends ScreenAdapter {
    private static final float HUB_X = 0.0f;
    private static final float HUB_Z = 0.0f;
    private static final String MAP_ASSET_KEY = "gltf-map-primary";
    private static final String MAP_MODEL_PATH = "models/sample.gltf";
    private static final float MAP_SCALE = 0.25f;
    private static final float MAP_Y_OFFSET = -1.2f;
    private static final float MAP_COLLIDER_MIN_SIZE = 0.1f;
    private static final float MAP_COLLIDER_MIN_HEIGHT = 0.12f;
    ...
    private final Environment gltfEnvironment = buildGltfEnvironment();
    ...
    private float[] mapCollisionDebugTriangles;
    ...
    private ModelAssetManager modelAssetManager;
    private SceneModel mapSceneModel;
    ...
    public GltfMapTestScene(MultiplayerLaunchConfig config) {
        this();
        setLaunchConfig(config);
    }

    private Environment buildGltfEnvironment() {
        Environment environment = new Environment();
        environment.set(new ColorAttribute(ColorAttribute.AmbientLight, 0.55f, 0.58f, 0.65f, 1f));
        environment.add(new DirectionalLight().set(0.9f, 0.88f, 0.82f, -0.35f, -1f, -0.2f));
        return environment;
    }

    private void loadGltfMap() {
        modelAssetManager = new ModelAssetManager();
        modelBatch = new ModelBatch();

        try {
            ModelData mapData = modelAssetManager.load(MAP_ASSET_KEY, MAP_MODEL_PATH);
            mapSceneModel = new SceneModel(mapData);
            mapSceneModel.setPosition(0f, MAP_Y_OFFSET, 0f);
            mapSceneModel.setScale(MAP_SCALE, MAP_SCALE, MAP_SCALE);
            if (!registerGltfMeshColliders(mapData)) {
                registerGltfGuardCollider(mapData);
            }
        } catch (ModelLoadException exception) {
            Gdx.app.error("GltfMapTestScene", "Failed to load map model: " + exception.getMessage(), exception);
            mapSceneModel = null;
        }
    }

    private boolean registerGltfMeshColliders(ModelData mapData) {
        MapCollisionBuilder.CollisionData collisionData = MapCollisionBuilder.build(
            mapData,
            MAP_SCALE,
            MAP_Y_OFFSET,
            MAP_COLLIDER_MIN_SIZE,
            MAP_COLLIDER_MIN_HEIGHT
        );

        if (collisionData == null || collisionData.colliders() == null || collisionData.colliders().isEmpty()) {
            mapCollisionDebugTriangles = null;
            return false;
        }

        mapCollisionDebugTriangles = collisionData.debugTriangles();
        for (BoundingBox collider : collisionData.colliders()) {
            worldColliders.add(new BoundingBox(collider));
        }

        return true;
    }

    private void registerGltfGuardCollider(ModelData mapData) {
        if (mapData == null || mapData.model() == null) {
            return;
        }

        BoundingBox mapBounds = new BoundingBox();
        new ModelInstance(mapData.model()).calculateBoundingBox(mapBounds);

        Vector3 dimensions = mapBounds.getDimensions(new Vector3()).scl(MAP_SCALE);
        Vector3 center = mapBounds.getCenter(new Vector3()).scl(MAP_SCALE);
        center.y += MAP_Y_OFFSET;
        if (dimensions.x < 0.1f || dimensions.z < 0.1f) {
            return;
        }

        float floorSlabY = (mapBounds.min.y * MAP_SCALE) + MAP_Y_OFFSET + 0.05f;
        float slabHeight = 0.1f;
        addCollider(center.x, floorSlabY, center.z, dimensions.x, slabHeight, dimensions.z);

        Vector3 scaledMin = new Vector3(mapBounds.min).scl(MAP_SCALE);
        Vector3 scaledMax = new Vector3(mapBounds.max).scl(MAP_SCALE);
        scaledMin.y += MAP_Y_OFFSET;
        scaledMax.y += MAP_Y_OFFSET;
        worldColliders.add(new BoundingBox(scaledMin, scaledMax));
    }
    ...
    private void renderGltfMap() {
        if (mapSceneModel == null || modelBatch == null) {
            return;
        }

        modelBatch.begin(camera);
        mapSceneModel.render(modelBatch, gltfEnvironment);
        modelBatch.end();
    }
