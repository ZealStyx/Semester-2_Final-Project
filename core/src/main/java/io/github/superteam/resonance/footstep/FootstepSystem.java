...
/** Material-aware locomotion sound emitter for the testbed. */
public final class FootstepSystem {
    ...
    public interface SurfaceResolver {
        SurfaceMaterial resolveSurface(Vector3 worldPosition);
    }
    ...
    private SurfaceResolver surfaceResolver;
    ...
    public void setSurfaceResolver(SurfaceResolver surfaceResolver) {
    ...
    private void emitFootstep(MovementState state, float nowSeconds) {
        ...
        String sourceNodeId = nodeIdResolver == null ? "center" : nodeIdResolver.resolveNodeId(worldPosition);
        SurfaceMaterial surfaceMaterial = surfaceResolver == null ? SurfaceMaterial.CONCRETE : surfaceResolver.resolveSurface(worldPosition);
        float materialMultiplier = surfaceMaterial == null ? 1f : surfaceMaterial.intensityMultiplier();
        ...
        SoundEventData soundEventData = new SoundEventData(
            ...
            sourceNodeId,
            worldPosition,
            intensityFor(state) * materialMultiplier,
            nowSeconds
        );
